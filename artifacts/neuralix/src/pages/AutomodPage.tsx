...
import {
  ShieldCheck, RefreshCw, Shield, Users, Plus, Trash2, UserCheck,
  Zap, AlertTriangle, Activity, Radio, Filter, Type, TriangleAlert
} from "lucide-react";
...
const TABS = [
  { id: "config", label: "Modulos" },
  { id: "filter", label: "Filtros de Contenido" },
...
const MODULE_COLOR: Record<string, string> = {
  ...
  AntiCaps: "border-cyan-500 bg-cyan-500/10 text-cyan-400",
  ...
  WordFilter: "border-amber-500 bg-amber-500/10 text-amber-400",
  AntiScam: "border-pink-500 bg-pink-500/10 text-pink-400",
  AntiLinks: "border-indigo-500 bg-indigo-500/10 text-indigo-400",
};
const ACTION_LABEL: Record<string, string> = { ban: "BAN", kick: "KICK", timeout: "TIMEOUT", mute: "TIMEOUT", strip: "STRIP ROLES", delete: "ELIMINAR", warn: "ADVERTENCIA" };
...
export default function AutomodPage() {
  ...
  useEffect(() => {
    ...
    evtSource.onmessage = (e) => {
      try {
        ...
        if (!alert.historical) {
          toast({ title: `${alert.module} — Detectado`, description: alert.description, variant: "destructive" });
        }
      } catch {}
    };
  ...
  const { data: config, isLoading } = useGetAntiraidConfig(guildId, {
    query: { queryKey: getGetAntiraidConfigQueryKey(guildId), enabled: !!guildId, refetchInterval: 10000, refetchIntervalInBackground: false }
  });
  const { data: stats } = useGetAntiraidStats(guildId, {
    query: { queryKey: getGetAntiraidStatsQueryKey(guildId), enabled: !!guildId && tab === "stats", refetchInterval: 5000, refetchIntervalInBackground: false }
  });
  ...
  const save = () => {
    update.mutate({ guildId, data: cfg }, {
      onSuccess: () => { toast({ title: "AutoMod guardado" }); qc.invalidateQueries({ queryKey: getGetAntiraidConfigQueryKey(guildId) }); },
  ...
  const handleUltra = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        toast({ title: "AUTOMOD ULTRA activado", description: "Todos los modulos en maxima proteccion" });
  ...
  const isUltraActive = cfg && cfg.enabled && cfg.antiJoin && cfg.antiAlt && cfg.antiBot && cfg.antiSpam && cfg.antiNuke && cfg.antiFlood && cfg.antiWebhook;
    ...
    <Layout guildId={guildId}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Proteccion avanzada contra raids, spam, bots, contenido inapropiado y ataques al servidor.</p>
        </div>
        {(tab === "config" || tab === "filter") && cfg && (
      ...
      {/* ── AUTOMOD ULTRA Banner ── */}
      {(tab === "config" || tab === "filter") && cfg && (
        <div className={cn(
          "max-w-3xl mb-6 rounded-xl border p-4 flex items-center justify-between gap-4",
          isUltraActive ? "border-green-500/40 bg-green-950/20" : "border-violet-500/30 bg-violet-950/20"
        )}>
          <div className="flex items-start gap-3">
            <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5", isUltraActive ? "bg-green-500/20" : "bg-violet-500/20")}>
              <Zap className={cn("w-5 h-5", isUltraActive ? "text-green-400" : "text-violet-400")} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <p className={cn("font-bold text-sm", isUltraActive ? "text-green-300" : "text-violet-300")}>AUTOMOD ULTRA</p>
                {isUltraActive && <span className="text-xs px-1.5 py-0.5 rounded-full bg-green-500/20 text-green-400 font-semibold">ACTIVO</span>}
              </div>
              <p className={cn("text-xs mt-0.5", isUltraActive ? "text-green-400/80" : "text-violet-400/80")}>
                {isUltraActive
                  ? "Servidor en maxima proteccion. Todos los modulos activos."
                  : "Activa todos los modulos de seguridad al maximo con un solo clic."}
              </p>
            </div>
          </div>
          {!isUltraActive && (
            <Button size="sm" onClick={handleUltra} disabled={ultraLoading}
              className={cn("flex-shrink-0 gap-1.5 font-bold transition-all",
                ultraConfirm
                  ? "bg-violet-600 hover:bg-violet-700 text-white border-violet-500 animate-pulse"
                  : "bg-violet-500/20 hover:bg-violet-500/30 text-violet-300 border border-violet-500/40"
              )}
              variant="ghost">
              {ultraLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <AlertTriangle className="w-3.5 h-3.5" />}
        ...
        <div className="max-w-3xl space-y-5">
          ...
          <div className="bg-card border border-card-border rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <Label className="font-semibold text-base">AutoMod activo</Label>
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={cfg.antiJoin} onCheckedChange={set("antiJoin")} />
            </div>
            {cfg.antiJoin && (
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={cfg.antiAlt} onCheckedChange={set("antiAlt")} />
            </div>
            {cfg.antiAlt && (
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={cfg.antiSpam} onCheckedChange={set("antiSpam")} />
            </div>
            {cfg.antiSpam && (
              <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div><Label className="font-semibold text-sm">AntiFlood</Label><p className="text-xs text-muted-foreground">Sanciona mensajes en rafaga en poco tiempo</p></div>
              <Switch checked={cfg.antiFlood ?? false} onCheckedChange={set("antiFlood")} />
            </div>
            {cfg.antiFlood && (
              <div className="grid grid-cols-3 gap-4">
                <div><Label className="text-xs mb-1.5 block">Limite de mensajes</Label><Input type="number" min={2} max={50} value={cfg.floodLimit ?? 5} onChange={setN("floodLimit")} /></div>
                <div><Label className="text-xs mb-1.5 block">Intervalo (segundos)</Label><Input type="number" min={1} max={60} value={cfg.floodInterval ?? 5} onChange={setN("floodInterval")} /></div>
                <div><Label className="text-xs mb-1.5 block">Accion</Label>
                  <NativeSelect value={cfg.floodAction || "mute"} onChange={set("floodAction")}>
                    <option value="warn">Advertencia</option><option value="mute">Timeout</option><option value="kick">Kick</option><option value="ban">Ban</option>
                  </NativeSelect>
                </div>
              </div>
            )}
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={cfg.antiBot} onCheckedChange={set("antiBot")} />
            </div>
            {cfg.antiBot && (
              <div>
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={cfg.antiNuke} onCheckedChange={set("antiNuke")} />
            </div>
            {cfg.antiNuke && (
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div><Label className="font-semibold text-sm">AntiWebhook Spam</Label><p className="text-xs text-muted-foreground">Detecta y actua contra la creacion masiva de webhooks</p></div>
              <Switch checked={cfg.antiWebhook || false} onCheckedChange={set("antiWebhook")} />
            </div>
            {cfg.antiWebhook && (
              <div className="grid grid-cols-2 gap-4">
                <div><Label className="text-xs mb-1.5 block">Webhooks en ventana</Label><Input type="number" min={2} value={cfg.webhookSpamThreshold ?? 3} onChange={setN("webhookSpamThreshold")} /></div>
                <div><Label className="text-xs mb-1.5 block">Ventana (segundos)</Label><Input type="number" min={5} value={cfg.webhookSpamInterval ?? 60} onChange={setN("webhookSpamInterval")} /></div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ── Filtros de Contenido Tab ── */}
      {tab === "filter" && cfg && (
        <div className="max-w-3xl space-y-5">
          {/* AntiLinks */}
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-7 h-7 rounded-lg bg-indigo-500/20 flex items-center justify-center"><Shield className="w-4 h-4 text-indigo-400" /></div>
              <div><Label className="font-semibold text-sm">AntiLinks</Label><p className="text-xs text-muted-foreground">Bloquea enlaces, invitaciones Discord y dominios maliciosos</p></div>
              <Switch checked={cfg.antiLinks} onCheckedChange={set("antiLinks")} className="ml-auto" />
            </div>
            {cfg.antiLinks && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <NativeSelect value={cfg.antiLinksAction || "delete"} onChange={set("antiLinksAction")}>
                      <option value="delete">Solo eliminar</option>
                      <option value="timeout">Eliminar + Silenciar</option>
                      <option value="kick">Eliminar + Kick</option>
                      <option value="ban">Eliminar + Ban</option>
                    </NativeSelect>
                  </div>
                ...
                <div>
                  <Label className="text-xs mb-1.5 block">Dominios bloqueados (vacio = bloquear todos salvo whitelist)</Label>
                ...
                <p className="text-xs text-muted-foreground">NSFW y dominios maliciosos (grabify, iplogger) siempre bloqueados automaticamente.</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-7 h-7 rounded-lg bg-cyan-500/20 flex items-center justify-center"><Type className="w-4 h-4 text-cyan-400" /></div>
              <div><Label className="font-semibold text-sm">AntiCaps</Label><p className="text-xs text-muted-foreground">Elimina mensajes con exceso de mayusculas</p></div>
              <Switch checked={cfg.antiCaps ?? false} onCheckedChange={set("antiCaps")} className="ml-auto" />
            </div>
            {cfg.antiCaps && (
              <div className="grid grid-cols-3 gap-4">
                <div><Label className="text-xs mb-1.5 block">Longitud minima del mensaje</Label><Input type="number" min={1} max={50} value={cfg.antiCapsMinLength ?? 10} onChange={setN("antiCapsMinLength")} /></div>
                <div><Label className="text-xs mb-1.5 block">% mayusculas para activar</Label><Input type="number" min={50} max={100} value={cfg.antiCapsPercent ?? 70} onChange={setN("antiCapsPercent")} /></div>
                <div><Label className="text-xs mb-1.5 block">Accion</Label>
                  <NativeSelect value={cfg.antiCapsAction || "delete"} onChange={set("antiCapsAction")}>
                    <option value="delete">Solo eliminar</option>
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-7 h-7 rounded-lg bg-amber-500/20 flex items-center justify-center"><Filter className="w-4 h-4 text-amber-400" /></div>
              <div><Label className="font-semibold text-sm">Filtro de Palabras</Label><p className="text-xs text-muted-foreground">Elimina mensajes con palabras prohibidas</p></div>
              <Switch checked={cfg.wordFilter ?? false} onCheckedChange={set("wordFilter")} className="ml-auto" />
            </div>
            {cfg.wordFilter && (
              <div className="space-y-3">
                <div>
                  <Label className="text-xs mb-1.5 block">Palabras prohibidas (una por linea o separadas por coma)</Label>
                  <Textarea
                    rows={4}
                    placeholder="insulto1, insulto2, frase prohibida"
                    value={Array.isArray(cfg.wordFilterList) ? cfg.wordFilterList.join(", ") : ""}
                    onChange={(e) => set("wordFilterList")(e.target.value.split(/[,\n]/).map((s: string) => s.trim()).filter(Boolean))}
                    className="resize-none"
                  />
                  <p className="text-xs text-muted-foreground mt-1">{Array.isArray(cfg.wordFilterList) ? cfg.wordFilterList.length : 0} palabras configuradas</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-xs mb-1.5 block">Accion</Label>
                    <NativeSelect value={cfg.wordFilterAction || "delete"} onChange={set("wordFilterAction")}>
                      <option value="delete">Solo eliminar</option>
                  ...
                  <div className="flex items-center gap-3 pt-5">
                    <Switch checked={cfg.wordFilterWildcard ?? false} onCheckedChange={set("wordFilterWildcard")} />
                    <div>
                      <Label className="text-sm">Coincidencia parcial</Label>
                      <p className="text-xs text-muted-foreground">Detecta palabras dentro de otras</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-card border border-card-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-7 h-7 rounded-lg bg-pink-500/20 flex items-center justify-center"><TriangleAlert className="w-4 h-4 text-pink-400" /></div>
              <div><Label className="font-semibold text-sm">AntiScam</Label><p className="text-xs text-muted-foreground">Detecta automaticamente patrones de estafa y phishing de Discord</p></div>
              <Switch checked={cfg.antiScam ?? false} onCheckedChange={set("antiScam")} className="ml-auto" />
            </div>
            {cfg.antiScam && (
              <div>
                <Label className="text-xs mb-1.5 block">Accion al detectar estafa</Label>
                <NativeSelect value={cfg.antiScamAction || "ban"} onChange={set("antiScamAction")}>
                  <option value="delete">Solo eliminar</option>
                  <option value="mute">Silenciar</option>
                  <option value="kick">Kick</option>
                  <option value="ban">Ban inmediato</option>
                </NativeSelect>
                <p className="text-xs text-muted-foreground mt-2">Detecta: Discord Nitro falso, phishing de cuentas, crypto scams, links de Steam fraudulentos.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {tab === "whitelist" && (
        <div className="max-w-2xl space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">Usuarios y roles exentos de todas las protecciones de AutoMod.</p>
            <Button size="sm" onClick={() => setShowWlForm(!showWlForm)}><Plus className="w-4 h-4 mr-1" /> Agregar</Button>
          </div>
          ...
          {showWlForm && (
            <div className="bg-card border border-card-border rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-xs mb-1.5 block">{wlForm.entityType === "role" ? "Rol *" : "ID de Discord *"}</Label>
                ...
                <div>
                  ...
                  <NativeSelect value={wlForm.entityType} onChange={(v) => setWlForm((f) => ({ ...f, entityType: v }))}>
                    <option value="user">Usuario</option><option value="role">Rol</option><option value="bot">Bot</option>
                  </NativeSelect>
                </div>
                <div><Label className="text-xs mb-1.5 block">Nombre (opcional)</Label><Input value={wlForm.name} onChange={(e) => setWlForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nombre" /></div>
                <div><Label className="text-xs mb-1.5 block">Razon (opcional)</Label><Input value={wlForm.reason} onChange={(e) => setWlForm((f) => ({ ...f, reason: e.target.value }))} placeholder="Razon" /></div>
              </div>
          ...
          {whitelist.length === 0 ? (
            <div className="text-center py-16 bg-card rounded-xl border border-card-border">
              ...
              <p className="text-sm text-muted-foreground mt-1">Agrega usuarios o roles exentos de AutoMod.</p>
            </div>
          ) : (
              ...
              {whitelist.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className={cn("w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0", entry.entityType === "role" ? "bg-purple-500/20" : "bg-green-500/20")}>
        ...
        <div className="max-w-3xl">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <Radio className="w-3.5 h-3.5" />{liveAlerts.length} eventos
            </div>
            {liveAlerts.length > 0 && (
              <button onClick={() => setLiveAlerts([])} className="ml-auto text-xs text-muted-foreground hover:text-foreground transition-colors">Limpiar</button>
            )}
          </div>
          ...
          {liveAlerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
              ...
              <p className="text-xs opacity-60">Las alertas aparecen aqui en tiempo real cuando AutoMod detecta actividad</p>
            </div>
          ) : (
            <div ref={alertsRef} className="flex flex-col gap-2">
              {liveAlerts.map((alert) => (
                <div key={alert.id} className={cn("border-l-2 rounded-lg p-4 flex items-start gap-4 transition-all", MODULE_COLOR[alert.module] ?? "border-muted bg-muted/20 text-muted-foreground", alert.historical ? "opacity-60" : "animate-in fade-in slide-in-from-top-1 duration-300")}>
                  ...
                  <span className={cn("shrink-0 text-[10px] font-black px-2 py-0.5 rounded border",
                    alert.action === "ban" ? "border-red-500/40 bg-red-500/20 text-red-400" :
                    alert.action === "kick" ? "border-orange-500/40 bg-orange-500/20 text-orange-400" :
